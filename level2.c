...
void level2(SDL_Surface *screen,int num,Game g)
{
    ...
    initEnnemi(&e);
    init_AllObjet_melon(&melon);
    init_AllObjet_key(&key);
    init_AllOobstacle_rocks(rocks);

    initialiser_score_melon(&melsc);
    ...
    init_minimap(&map,&Backg,&character, &e,2);
    ...
    {
        load_jeu(&Backg, &e, &character,&melon,&key,&mytime);
...
//manette
    ...
    afficherimgLevel(imgLevel,screen,1);
    while(boucle)
    {
        ...
        afficherALLObjets(&melon,character,screen, Backg);
        afficherALLObjets(&key,character,screen, Backg);
        afficherALLObstacles(rocks, screen, Backg);



        afficherScore(&melsc,&melon,screen);
        ...
        while(SDL_PollEvent(&event))
        {
            ...
            case SDL_KEYDOWN:
                if(event.key.keysym.sym ==SDLK_ESCAPE)
                {
                    ...
                    sauvegarde_jeu(Backg, e,character,melon,key,&mytime);
                    submenuS(screen);
        ...
        for(int i=0; i<6; i++)
        {
            handle_collision(&character, &rocks[i],&Backg);
        }
        ...
        afficher_score(screen, score, temps_debut, temps_fin,&melon,&key,&s);
