...
void level3(SDL_Surface *screen,int num,Game g)
{
    ...
    initEnnemi(&e);
    init_AllObjet_bottle(&bottle);
    init_AllObjet_key(&key);
    init_AllOobstacle_wood(wood);

    initialiser_score_bottle(&wdsc);
    ...
    init_minimap(&map,&Backg,&character, &e,3);
    ...
    if(num!=0)
    {
        load_jeu(&Backg, &e, &character,&bottle,&key,&mytime);
...
//manette
    ...
    afficherimgLevel(imgLevel,screen,2);
    ...
    while(boucle)
    {
        ...
        afficherALLObjets(&bottle,character,screen, Backg);
        afficherALLObjets(&key,character,screen, Backg);
        afficherALLObstacles(wood, screen, Backg);



        afficherScore(&wdsc,&bottle,screen);
        ...
        while(SDL_PollEvent(&event))
        {
            ...
            case SDL_KEYDOWN:
                if(event.key.keysym.sym ==SDLK_ESCAPE)
                {
                    ...
                    sauvegarde_jeu(Backg, e,character,bottle,key,&mytime);
                    submenuS(screen);
        ...
        for(int i=0; i<6; i++)
        {
            handle_collision(&character, &wood[i],&Backg);
        }
        ...
        afficher_score(screen, score, temps_debut, temps_fin,&bottle,&key,&s);
